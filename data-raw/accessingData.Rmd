...
# Option 1: 
# Summarized by gene symbol with latest annotation and using the average of 
# all probes associated with a gene as the summary expression. The method combines
# expressionSets for you by 
sumMx <- gl$getGEMatrix(matrixName = mats$name, 
                        outputType = "summary", 
                        annotation = "latest")
sumEset <- Biobase::exprs(sumMx)
...
Write the two datasets to objects in a target filepath

```{r write-objects-to-rda}
dirPath <- "/home/ehenrich/R/IS2/"
save(hai, file = paste0(dirPath, "/IS2_hai.rda"))
save(exprs, file = paste0(dirPath, "/IS2_exprs.rda")) # ~ 80 MB
```
